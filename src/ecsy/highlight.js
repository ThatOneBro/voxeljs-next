...
export class HighlightSystem extends System {
  init() {}
  traceRayAtScreenCoords(stageRot, stagePos, domElement, camera, chunkManager, pt, distance) {
    const ray = new Ray();
        ...
        this.queries.highlights.results.forEach(ent => {
          ...
          this.queries.inputs.results.forEach(ent => {
            ...
            if (input.state[InputFrame.CREATE_AT_CURSOR] === true) {
              ...
              landscape.chunkManager.setVoxelAtCoordinates(
                pos,
                active.getComponent(ActiveBlock).type,
              );
            }
